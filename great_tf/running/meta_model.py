...
class VarMisuseModel(tf.keras.layers.Layer):
    ...
    def build(self, _):
        ...
        self.prediction = tf.keras.layers.Dense(2) # Only consider location # Two pointers: bug location and repair
    ...
    # @tf.function(input_signature=[tf.TensorSpec(shape=(None, 1, None), dtype=tf.float32), tf.TensorSpec(shape=(None, None), dtype=tf.int32), tf.TensorSpec(shape=(None,), dtype=tf.int32), tf.TensorSpec(shape=(None, None), dtype=tf.int32), tf.TensorSpec(shape=(None, None), dtype=tf.int32)])
    def get_loss(self, predictions, token_mask, error_locations, repair_targets, repair_candidates):
        ...
        # ignore other losses and accuracies
        # target_loss = 0.0
        # target_loc_acc = 0.0
        # joint_acc = 0.0
