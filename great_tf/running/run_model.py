...
def evaluate(
    ...
    for batch in data.batcher(mode="dev" if is_heldout else "eval"):
        ...
        pointer_preds += (1.0 - tf.expand_dims(seq_mask, 1)) * tf.float32.min

        loc_predictions = pointer_preds[:, 0]
        loc_pred = tf.argmax(loc_predictions, axis=-1)

        # pointer_logits = pointer_preds[:, 1]
        # candidate_mask = tf.scatter_nd(
        #     repair_candidates,
        #     tf.ones(tf.shape(repair_candidates)[0]),
        ...
        #     repair_targets,
        #     tf.ones(tf.shape(repair_targets)[0]),
        #     tf.shape(pointer_probs),
        # )
        ...
        for i in range(batch_size):
            result = {
                "loc_pred": loc_pred[i].numpy(),
                # "target_probs": target_probs[i].numpy(),
                # "pointer_probs": pointer_probs[i].numpy().tolist(),
                "gt_error_loc": error_loc[i].numpy(),
