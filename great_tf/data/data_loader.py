...
class DataLoader:
    ...
    def batcher(self, mode="train"):
        ...
        if mode == "train" and self.config.get("typechecking_oversample", False):
            print("Use dataset with oversampling")
            dataset = tf.data.Dataset.list_files(
                data_path + "/*.txt*", shuffle=mode != "eval", seed=42
            )
        else:
            print("Use normal dataset")
            dataset = tf.data.Dataset.list_files(
                data_path + "/*__VARIABLE_MISUSE__SStuB.txt*", shuffle=mode != "eval", seed=42
            )

        dataset = dataset.interleave(
            lambda x: (
                tf.data.TextLineDataset(x, num_parallel_reads=16).shuffle(
                    buffer_size=1000
                )
                if mode != "eval"
    ...
    def to_batch(self, sample_generator, mode):
        ...
        def make_batch(buffer):
            ...
            target_batches = tf.repeat(
                tf.range(batch_dim), [len(targets) for targets in batch[3]]
            )
            repair_targets = tf.cast(
                tf.concat(batch[3], axis=0), "int32"
            )  # Explicitly cast to int32 since repair targets can be empty, in which case TF defaults to float32
            repair_targets = tf.stack([target_batches, repair_targets], axis=1)

            candidates_batches = tf.repeat(
                tf.range(batch_dim), [len(candidates) for candidates in batch[4]]
            )
            repair_candidates = tf.concat(batch[4], axis=0)
            repair_candidates = tf.stack(
                [candidates_batches, repair_candidates], axis=1
            )
            sample_ids = tf.constant(batch[5], dtype=tf.dtypes.int32)
        ...
        for line in sample_generator:
            ...
            if "id" not in json_sample:
                json_sample["id"] = counter
                counter += 1
            ...
            if (
                ...
                and self.typecheck_dataset[json_sample["id"]]["type_check_failed"]
            ):
                continue
            ...
            if (
                ...
                and not self.random_filter_dataset[json_sample["id"]]["included"]
            ):
                continue
